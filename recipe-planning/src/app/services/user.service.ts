...
export class UserService {
    ...
    async followUser(currentUser: User, targetUser: User) {
        if (!currentUser.id || !targetUser.id || currentUser.id.trim() === '' || targetUser.id.trim() === '') {
            console.error('(followUserFunction)Invalid user IDs for follow operation:', {
    ...
    getFollowing(user: User): Observable<string[]> {
        ...
        return from(getDocs(followingRef).then(snapshot =>
    ...
    getFollowers(user: User): Observable<string[]> {
        ...
        return from(getDocs(followersRef).then(snapshot =>
            snapshot.docs.map(doc => doc.data()['userId'])
        ));
    }
    ...
    getUserById(id: string): Observable<User> {
        ...
        const userRef = doc(this.firestore, 'users', id);
        return from(getDoc(userRef).then(snapshot => {
            const data = snapshot.data();
            if (!data) throw new Error('User not found');
            return this.createUser(
                data['email'],
    ...
    loadFollowing(user: User): Observable<void> {
        ...
        return from(getDoc(userRef).then(snapshot => {
            ...
            if (data && data['following']) {
                user.following = data['following'].filter((id: string) => id && id.trim() !== '');
